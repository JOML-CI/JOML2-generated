...
/**
 * Read-only view of a plane of single-precision {@code float} components.
 * <p>
 * Declares the accessors, queries, store methods and dest-form operations that read but never
 * mutate the receiver; the mutable counterpart is {@link FloatPlane}. APIs that only read a plane
 * should accept {@code FloatPlaneR}, so callers can pass mutable instances without exposing them to
 * modification.
 * <p>
 * Arguments of type {@code FloatPlaneR} must be instances created by the library ({@link Joml}
...
public interface FloatPlaneR {
    ...
    public static FloatPlaneR ZERO() { return Holder.ZERO; }
     ...
     * @return dest
     */
    FloatPlane setNormal(Float3R n, @Mutated FloatPlane dest);
     ...
     * @return dest
     */
    DoublePlane setNormal(Float3R n, @Mutated DoublePlane dest);
     ...
     * @return dest
     */
    FloatPlane setNormal(float x, float y, float z, @Mutated FloatPlane dest);
     ...
     * @return dest
     */
    DoublePlane setNormal(float x, float y, float z, @Mutated DoublePlane dest);
     ...
     * @return dest
     */
    DoublePlane toDouble(@Mutated DoublePlane dest);
     ...
     * stays within the {@code float} range: the magnitude of the normal {@code (a, b, c)} must lie
     * roughly between {@code 1e-19} and {@code 1.8e19}. Rescale inputs outside that band first.
     *
     * @param dest will hold the result
     * @return dest
     */
    FloatPlane normalize(@Mutated FloatPlane dest);
     ...
     *        variants only)
     */
    FloatPlane storeUnsafe(long address);
     ...
     *        variants only)
     */
    FloatPlane storeDoubleUnsafe(long address);
     ...
     */
    boolean equalsEpsilon(FloatPlaneR other, float epsilon);
     ...
     */
    float signedDistance(float pX, float pY, float pZ);
     ...
     *        plane normal points to, and a true distance even when that normal is not unit length
     */
    float signedDistance(Float3R p);
     ...
     * @return {@code dest}
     */
    Float3 projectPoint(float pX, float pY, float pZ, @Mutated Float3 dest);
     ...
     * @return {@code dest}
     */
    Float3 projectPoint(Float3R p, @Mutated Float3 dest);
     ...
     */
    boolean containsPoint(float pX, float pY, float pZ, float epsilon);
     ...
     *        {@code false} otherwise
     */
    boolean containsPoint(Float3R p, float epsilon);
     ...
     *        otherwise
     */
    boolean intersectsAABB(FloatAABBR box);
    ...
    final class Holder {
        private Holder() {}
        static final FloatPlaneR ZERO = Joml.floatPlane(0, 0, 0, 0);
    }
}
