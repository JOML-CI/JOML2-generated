...
public final class IntersectionGenf {
    ...
    public static Float3 findClosestPointOnLineSegment(float aX, float aY, float aZ, float bX, float bY, float bZ, float pX, float pY, float pZ, Float3 result) {
        ...
        float _t1 = bZ - aZ;
        float _t4 = bY - aY;
        float _t14 = Math.clamp(Math.fma(_t1, pZ - aZ, Math.fma(_t0, pX - aX, _t4 * (pY - aY))) / Math.max(Math.fma(_t1, _t1, Math.fma(_t0, _t0, _t4 * _t4)), 1.4E-45f), 0.0f, 1.0f);
    ...
    public static boolean testLineSegmentSphere(float p0X, float p0Y, float p0Z, float p1X, float p1Y, float p1Z, float centerX, float centerY, float centerZ, float radiusSquared) {
        ...
        float _t3 = p1X - p0X;
        float _t5 = p1Y - p0Y;
        float _t17 = Math.clamp(Math.fma(centerZ - p0Z, _t0, Math.fma(centerX - p0X, _t3, (centerY - p0Y) * _t5)) / Math.max(Math.fma(_t0, _t0, Math.fma(_t3, _t3, _t5 * _t5)), 1.4E-45f), 0.0f, 1.0f);
