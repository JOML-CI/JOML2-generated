...
public final class FloatOBBImpl implements FloatOBB {
    ...
    public FloatOBB transform(Float3x4R m, @Mutated FloatOBB dest) {
        ...
        float[] dd = ((FloatOBBImpl) dest).data;
        float _t18 = Math.fma(mData[2], sd[5], Math.fma(mData[0], sd[3], mData[1] * sd[4]));
        float _t19 = Math.fma(mData[10], sd[5], Math.fma(mData[8], sd[3], mData[9] * sd[4]));
        float _t20 = Math.fma(mData[6], sd[5], Math.fma(mData[4], sd[3], mData[5] * sd[4]));
        float _t21 = Math.fma(mData[10], sd[8], Math.fma(mData[8], sd[6], mData[9] * sd[7]));
        float _t22 = Math.fma(mData[2], sd[8], Math.fma(mData[0], sd[6], mData[1] * sd[7]));
        float _t23 = Math.fma(mData[6], sd[8], Math.fma(mData[4], sd[6], mData[5] * sd[7]));
        float _t24 = Math.fma(mData[10], sd[11], Math.fma(mData[8], sd[9], mData[9] * sd[10]));
        float _t25 = Math.fma(mData[2], sd[11], Math.fma(mData[0], sd[9], mData[1] * sd[10]));
        ...
        dd[10] = _t55;
        dd[11] = _t56;
        float _buf2 = Math.fma(sd[14], Math.abs(Math.fma(_t24, _t33, Math.fma(_t25, _t31, _t26 * _t32))), Math.fma(sd[12], Math.abs(Math.fma(_t19, _t33, Math.fma(_t18, _t31, _t20 * _t32))), sd[13] * Math.abs(_t36)));
        float _buf3 = Math.fma(sd[14], Math.abs(Math.fma(_t24, _t47, Math.fma(_t25, _t45, _t26 * _t46))), Math.fma(sd[12], Math.abs(Math.fma(_t19, _t47, Math.fma(_t18, _t45, _t20 * _t46))), sd[13] * Math.abs(Math.fma(_t21, _t47, Math.fma(_t22, _t45, _t23 * _t46)))));
    ...
    public FloatOBB transform(Float4x4R m, @Mutated FloatOBB dest) {
        ...
        float[] dd = ((FloatOBBImpl) dest).data;
        float _t18 = Math.fma(mData[8], sd[5], Math.fma(mData[0], sd[3], mData[4] * sd[4]));
        float _t19 = Math.fma(mData[10], sd[5], Math.fma(mData[2], sd[3], mData[6] * sd[4]));
        float _t20 = Math.fma(mData[9], sd[5], Math.fma(mData[1], sd[3], mData[5] * sd[4]));
        float _t21 = Math.fma(mData[10], sd[8], Math.fma(mData[2], sd[6], mData[6] * sd[7]));
        float _t22 = Math.fma(mData[8], sd[8], Math.fma(mData[0], sd[6], mData[4] * sd[7]));
        float _t23 = Math.fma(mData[9], sd[8], Math.fma(mData[1], sd[6], mData[5] * sd[7]));
        float _t24 = Math.fma(mData[10], sd[11], Math.fma(mData[2], sd[9], mData[6] * sd[10]));
        float _t25 = Math.fma(mData[8], sd[11], Math.fma(mData[0], sd[9], mData[4] * sd[10]));
