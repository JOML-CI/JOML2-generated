...
public final class Float4Impl implements Float4 {
    ...
    @Mutated public Float4 makeUniformDirection(float u, float v, float w) {
        ...
        dd[0] = (float) Math.cosFromSin(_t4, _t1) * _t5;
        dd[1] = _t4 * _t5;
        dd[2] = (float) Math.cosFromSin(_t6, _t3) * _t0;
        dd[3] = _t6 * _t0;
        return this;
    }
    ...
    public Float4 project(float ontoX, float ontoY, float ontoZ, float ontoW, @Mutated Float4 dest) {
        float[] sd = this.data;
        float[] dd = ((Float4Impl) dest).data;
        float _sp0 = Math.fma(ontoW, sd[3], Math.fma(ontoZ, sd[2], Math.fma(ontoX, sd[0], ontoY * sd[1]))) / Math.fma(ontoW, ontoW, Math.fma(ontoZ, ontoZ, Math.fma(ontoX, ontoX, ontoY * ontoY)));
        dd[0] = ontoX * _sp0;
        dd[1] = ontoY * _sp0;
        dd[2] = ontoZ * _sp0;
        dd[3] = ontoW * _sp0;
