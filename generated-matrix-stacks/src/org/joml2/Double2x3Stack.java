...
import org.joml2.internal.types.Double2x3Impl;

/**
 * Fixed-size {@link Double2x3} stack - port of JOML 1's
 ...
 * {@link Double2x3} operations apply to it directly. Calling
 * {@link #pushMatrix()} snapshots the current state into an internal
 * {@code Double2x3[]} array; {@link #popMatrix()} restores from it.</p>
 *
 ...
 * {@link Double2x3} holding the same elements), {@code hashCode} hashes the top
 ...
 */
public class Double2x3Stack extends Double2x3Impl {

    /** Internal stack of saved matrices. Length is {@code stackSize - 1};
     *  the inherited {@code Double2x3} occupies the implicit top slot. */
    private final Double2x3[] mats;

    /** Index of the next free slot in {@link #mats}. {@code 0} == bottom of stack. */
    private int curr;

    /**
     * Create a new {@link Double2x3Stack} of the given size.
     ...
     */
    public Double2x3Stack(int stackSize) {
        if (stackSize < 1)
            throw new IllegalArgumentException("stackSize must be >= 1");
        mats = new Double2x3[stackSize - 1];
        // Eager allocation keeps runtime push/pop allocation-free.
        for (int i = 0; i < mats.length; i++)
            mats[i] = Joml.double2x3();
    }
     ...
     */
    public Double2x3Stack pushMatrix() {
        if (curr == mats.length)
     ...
     */
    public Double2x3Stack popMatrix() {
        if (curr == 0)
