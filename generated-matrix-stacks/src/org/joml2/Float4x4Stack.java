...
/**
 * Fixed-size {@link Float4x4} stack - port of JOML 1's
 ...
 * {@link Float4x4} operations apply to it directly. Calling
 * {@link #pushMatrix()} snapshots the current state into an internal
 * {@code Float4x4[]} array; {@link #popMatrix()} restores from it.</p>
 *
 ...
 * {@link Float4x4} holding the same elements), {@code hashCode} hashes the top
 ...
 */
public class Float4x4Stack extends Float4x4Impl {

    /** Internal stack of saved matrices. Length is {@code stackSize - 1};
     *  the inherited {@code Float4x4} occupies the implicit top slot. */
    private final Float4x4[] mats;

    /** Index of the next free slot in {@link #mats}. {@code 0} == bottom of stack. */
    private int curr;

    /**
     * Create a new {@link Float4x4Stack} of the given size.
     ...
     */
    public Float4x4Stack(int stackSize) {
        if (stackSize < 1)
            throw new IllegalArgumentException("stackSize must be >= 1");
        mats = new Float4x4[stackSize - 1];
        // Eager allocation keeps runtime push/pop allocation-free.
        for (int i = 0; i < mats.length; i++)
            mats[i] = Joml.float4x4();
    }
     ...
     */
    public Float4x4Stack pushMatrix() {
        if (curr == mats.length)
     ...
     */
    public Float4x4Stack popMatrix() {
        if (curr == 0)
