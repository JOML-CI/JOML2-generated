...
import org.joml2.internal.types.Double2x2Impl;

/**
 * Fixed-size {@link Double2x2} stack - port of JOML 1's
 ...
 * {@link Double2x2} operations apply to it directly. Calling
 * {@link #pushMatrix()} snapshots the current state into an internal
 * {@code Double2x2[]} array; {@link #popMatrix()} restores from it.</p>
 *
...
public class Double2x2Stack extends Double2x2Impl {
     ...
     *  the inherited {@code Double2x2} occupies the implicit top slot. */
    private final Double2x2[] mats;

    /** Index of the next free slot in {@link #mats}. {@code 0} == bottom of stack. */
    private int curr;

    /**
     * Create a new {@link Double2x2Stack} of the given size.
     ...
     */
    public Double2x2Stack(int stackSize) {
        if (stackSize < 1)
            throw new IllegalArgumentException("stackSize must be >= 1");
        mats = new Double2x2[stackSize - 1];
        // Eager allocation keeps runtime push/pop allocation-free.
        for (int i = 0; i < mats.length; i++)
            mats[i] = Joml.double2x2();
    }
     ...
     */
    public Double2x2Stack pushMatrix() {
        if (curr == mats.length)
     ...
     */
    public Double2x2Stack popMatrix() {
        if (curr == 0)
