...
import org.joml2.internal.types.Float3x3Impl;

/**
 * Fixed-size {@link Float3x3} stack - port of JOML 1's
 ...
 * {@link Float3x3} operations apply to it directly. Calling
 * {@link #pushMatrix()} snapshots the current state into an internal
 * {@code Float3x3[]} array; {@link #popMatrix()} restores from it.</p>
 *
 ...
 * {@link Float3x3} holding the same elements), {@code hashCode} hashes the top
 ...
 */
public class Float3x3Stack extends Float3x3Impl {

    /** Internal stack of saved matrices. Length is {@code stackSize - 1};
     *  the inherited {@code Float3x3} occupies the implicit top slot. */
    private final Float3x3[] mats;

    /** Index of the next free slot in {@link #mats}. {@code 0} == bottom of stack. */
    private int curr;

    /**
     * Create a new {@link Float3x3Stack} of the given size.
     ...
     */
    public Float3x3Stack(int stackSize) {
        if (stackSize < 1)
            throw new IllegalArgumentException("stackSize must be >= 1");
        mats = new Float3x3[stackSize - 1];
        // Eager allocation keeps runtime push/pop allocation-free.
        for (int i = 0; i < mats.length; i++)
            mats[i] = Joml.float3x3();
    }
     ...
     */
    public Float3x3Stack pushMatrix() {
        if (curr == mats.length)
     ...
     */
    public Float3x3Stack popMatrix() {
        if (curr == 0)
