...
import org.joml2.internal.types.Float2x2Impl;

/**
 * Fixed-size {@link Float2x2} stack - port of JOML 1's
 ...
 * {@link Float2x2} operations apply to it directly. Calling
 * {@link #pushMatrix()} snapshots the current state into an internal
 * {@code Float2x2[]} array; {@link #popMatrix()} restores from it.</p>
 *
 ...
 * {@link Float2x2} holding the same elements), {@code hashCode} hashes the top
 ...
 */
public class Float2x2Stack extends Float2x2Impl {

    /** Internal stack of saved matrices. Length is {@code stackSize - 1};
     *  the inherited {@code Float2x2} occupies the implicit top slot. */
    private final Float2x2[] mats;

    /** Index of the next free slot in {@link #mats}. {@code 0} == bottom of stack. */
    private int curr;

    /**
     * Create a new {@link Float2x2Stack} of the given size.
     ...
     */
    public Float2x2Stack(int stackSize) {
        if (stackSize < 1)
            throw new IllegalArgumentException("stackSize must be >= 1");
        mats = new Float2x2[stackSize - 1];
        // Eager allocation keeps runtime push/pop allocation-free.
        for (int i = 0; i < mats.length; i++)
            mats[i] = Joml.float2x2();
    }
     ...
     */
    public Float2x2Stack pushMatrix() {
        if (curr == mats.length)
     ...
     */
    public Float2x2Stack popMatrix() {
        if (curr == 0)
