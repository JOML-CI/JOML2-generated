...
/**
 * Generated implementation of {@link FloatPlane} backed by a {@code float[]} array.
 * <p>
...
public final class FloatPlaneImpl implements FloatPlane {
    ...
    public float distanceToPoint(float pX, float pY, float pZ) {
        float[] sd = this.data;
        return (1.0f / (float) Math.sqrt(Math.fma(sd[2], sd[2], Math.fma(sd[0], sd[0], sd[1] * sd[1])))) * Math.abs(Math.fma(pX, sd[0], Math.fma(pY, sd[1], Math.fma(pZ, sd[2], sd[3]))));
    }
