...
public final class Float2Impl implements Float2 {
    ...
    public float angleBetween(float otherX, float otherY) {
        return (float) Math.acos(Math.min(1.0f, Math.max(-1.0f, (otherX * this.x + otherY * this.y) * (1.0f / (float) Math.sqrt(this.x * this.x + this.y * this.y)) * (1.0f / (float) Math.sqrt(otherX * otherX + otherY * otherY)))));
    }
    ...
    public Float2 project(float ontoX, float ontoY, @Mutated Float2 dest) {
        Float2Impl d = (Float2Impl) dest;
        float _t4 = ontoX * this.x + ontoY * this.y;
        float _t5 = ontoX * ontoX + ontoY * ontoY;
