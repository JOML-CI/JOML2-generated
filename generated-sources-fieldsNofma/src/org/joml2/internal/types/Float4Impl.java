...
public final class Float4Impl implements Float4 {
    ...
    public Float4 catmullRom(float p1X, float p1Y, float p1Z, float p1W, float p2X, float p2Y, float p2Z, float p2W, float p3X, float p3Y, float p3Z, float p3W, float t, @Mutated Float4 dest) {
        ...
        d.w = 0.5f * (p1W + p1W + t * (p2W - this.w) + ((-5.0f * p1W + (this.w + this.w + (4.0f * p2W - p3W))) * _t0 + (3.0f * p1W + (p3W - this.w) - 3.0f * p2W) * _t1));
        return d;
    }
    ...
    public Float4 catmullRomTangent(float p1X, float p1Y, float p1Z, float p1W, float p2X, float p2Y, float p2Z, float p2W, float p3X, float p3Y, float p3Z, float p3W, float t, @Mutated Float4 dest) {
        ...
        d.w = 0.5f * (_sp0 * (-5.0f * p1W + (this.w + this.w + (4.0f * p2W - p3W))) + (_sp1 * (3.0f * p1W + (p3W - this.w) - 3.0f * p2W) + (p2W - this.w)));
        return d;
    }
    ...
    public float angleBetween(float otherX, float otherY, float otherZ, float otherW) {
        ...
        float _ct0 = _t12 * _t12 + _t13 * _t13 + _t14 * _t14 + _t15 * _t15 + _t16 * _t16 + _t17 * _t17;
        if (!(_ct0 > 1.1754944E-38f && _ct0 < Float.POSITIVE_INFINITY)) return angleBetween_degenerate(otherX, otherY, otherZ, otherW);
        return (float) Math.atan2((float) Math.sqrt(_ct0), otherX * this.x + otherY * this.y + otherZ * this.z + otherW * this.w);
    }
    ...
    private float angleBetween_degenerate(float otherX, float otherY, float otherZ, float otherW) {
        ...
        float _t16 = otherY * _t6;
        float _t17 = this.x * _t7;
        float _t18 = otherX * _t6;
        float _t19 = this.y * _t7;
        float _t20 = otherZ * _t6;
        float _t21 = this.z * _t7;
        float _t22 = otherW * _t6;
        float _t23 = this.w * _t7;
    ...
    public float distance(float otherX, float otherY, float otherZ, float otherW) {
        ...
        float _t3 = this.w - otherW;
        return (float) Math.sqrt(_t0 * _t0 + _t1 * _t1 + _t2 * _t2 + _t3 * _t3);
    }
    ...
    public Float4 faceforward(float IX, float IY, float IZ, float IW, float NrefX, float NrefY, float NrefZ, float NrefW, @Mutated Float4 dest) {
        Float4Impl d = (Float4Impl) dest;
        float _t6 = IX * NrefX + IY * NrefY + IZ * NrefZ + IW * NrefW;
        if (_t6 < 0.0f) {
            d.x = this.x;
            d.y = this.y;
            d.z = this.z;
    ...
    public Float4 normalizeMul(float length, @Mutated Float4 dest) {
        Float4Impl d = (Float4Impl) dest;
        float _t6 = this.x * this.x + this.y * this.y + this.z * this.z + this.w * this.w;
        float _t8 = length * (1.0f / (float) Math.sqrt(_t6));
        if (_t6 != 0.0f) {
            d.x = this.x * _t8;
            d.y = this.y * _t8;
            d.z = this.z * _t8;
    ...
    public Float4 project(float ontoX, float ontoY, float ontoZ, float ontoW, @Mutated Float4 dest) {
        Float4Impl d = (Float4Impl) dest;
        float _sp0 = (ontoX * this.x + ontoY * this.y + ontoZ * this.z + ontoW * this.w) / (ontoX * ontoX + ontoY * ontoY + ontoZ * ontoZ + ontoW * ontoW);
        d.x = ontoX * _sp0;
        d.y = ontoY * _sp0;
        d.z = ontoZ * _sp0;
        d.w = ontoW * _sp0;
    ...
    public Float4 projectOnPlane(float normalX, float normalY, float normalZ, float normalW, @Mutated Float4 dest) {
        Float4Impl d = (Float4Impl) dest;
        float _t6 = normalX * this.x + normalY * this.y + normalZ * this.z + normalW * this.w;
    ...
    public Float4 reflect(float normalX, float normalY, float normalZ, float normalW, @Mutated Float4 dest) {
        Float4Impl d = (Float4Impl) dest;
        float _t7 = 2.0f * (normalX * this.x + normalY * this.y + normalZ * this.z + normalW * this.w);
    ...
    public Float4 refract(float normalX, float normalY, float normalZ, float normalW, float eta, @Mutated Float4 dest) {
        Float4Impl d = (Float4Impl) dest;
        float _t7 = normalX * this.x + normalY * this.y + normalZ * this.z + normalW * this.w;
        float _t12 = 1.0f - (1.0f - _t7 * _t7) * eta * eta;
        float _t15 = eta * _t7 + (float) Math.sqrt(Math.max(0.0f, _t12));
        if (_t12 >= 0.0f) {
    ...
    public Float4 smoothstep(float edge0, float edge1, @Mutated Float4 dest) {
        ...
        d.x = (3.0f - (_t13 + _t13)) * _t13 * _t13;
        d.y = (3.0f - (_t14 + _t14)) * _t14 * _t14;
        d.z = (3.0f - (_t15 + _t15)) * _t15 * _t15;
        d.w = (3.0f - (_t16 + _t16)) * _t16 * _t16;
        return d;
    }
    ...
    public Float4 smoothstep(float edge0X, float edge0Y, float edge0Z, float edge0W, float edge1X, float edge1Y, float edge1Z, float edge1W, @Mutated Float4 dest) {
        ...
        d.x = (3.0f - (_t16 + _t16)) * _t16 * _t16;
        d.y = (3.0f - (_t17 + _t17)) * _t17 * _t17;
        d.z = (3.0f - (_t18 + _t18)) * _t18 * _t18;
        d.w = (3.0f - (_t19 + _t19)) * _t19 * _t19;
        return d;
    }
    ...
    public Float4 preMul(Float4x4R mat, @Mutated Float4 dest) {
        Float4Impl d = (Float4Impl) dest;
        float _buf0 = mat.m00() * this.x + mat.m01() * this.y + mat.m02() * this.z + mat.m03() * this.w;
        float _buf1 = mat.m10() * this.x + mat.m11() * this.y + mat.m12() * this.z + mat.m13() * this.w;
        float _buf2 = mat.m20() * this.x + mat.m21() * this.y + mat.m22() * this.z + mat.m23() * this.w;
